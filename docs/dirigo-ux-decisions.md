...
| **E — Agent** 🔨 | 5.1, 5.3, 5.2, 5.5, 5.6, 4.3, agent-option display-staleness fix (§9.9) | 5.1 blocks 5.3; depends on A2.5 (complete). Staleness fix rides along with 5.5/5.6 — same modal, same visit. Build order (6 checkpoints): **5.6 wrong-tab → staleness fix → 5.1+5.2 → 5.3 → 5.5 → 4.3**. Decisions with Eric: Q1→B (staleness = warn + acknowledge + record deltas), Q2→A (wide EV bands), Q3→A (shared panel not overlay), Q4→refactor `deriveWinLikelihood()` in place. **Step 1 (5.6) done 2026-09-02**: shared `SubmitResultPanel` on Agent+Output, recommended-vs-chosen, `recommended_bid`/`recommended_margin_pct` on the bid record; wrong-tab §9.9 defect closed. **Step 2 (staleness) done 2026-09-02**: `agentStaleness.js` + `AgentStalenessWarning` (OR threshold, $500/0.5%), interim caveat removed from `AgentPage` + `BidSummaryPage`, acknowledge checkbox gates Confirm (composes with the override gate), `inputs_stale_at_submit`/`stale_bid_price_delta`/`stale_direct_cost_delta` on the record; §9.9 staleness defect closed; spec renamed `agent-staleness.spec.js`. **Step 3 (5.1 EV + 5.2 attribution) done 2026-09-02**: `src/state/expectedValue.js` (wide bands, Q2=A), `deriveWinLikelihood()` refactored in place to a breakdown (Q4), EV range + always-visible caveat + click-to-expand attribution on the option cards; `agent-expected-value.spec.js`. **Step 4 (5.3 what-if slider) done 2026-09-02**: `WhatIfSlider.jsx` + `interpolateAtBid()` (endpoint-exact, no lerp at an anchor), below the cards, isolated from card selection; `agent-what-if-slider.spec.js`. **Step 5 (5.5 override reason) done 2026-09-02**: reason chips + free text in the finalize modal when `selected !== 'recommended'`, `override_reason_chips`/`override_reason_text` on the bid record ([]/'' not null), hand-verified; `agent-override-reason.spec.js`. Remaining: 4.3 top cost drivers (close-out). |
