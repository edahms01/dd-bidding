...
**Visibility update (2026-09-02, Phase D Step 3) — same "the phase's own change made this more visible" pattern as the 2026-08-28 note above, now on a second surface.** Phase D's mobile Bid Summary (`BidSummaryPage.jsx`) reads the identical `state.ui.agent.cachedResult` object `AgentPage` does, through the same shared store, and carries the same one-line caveat above the same stacked options — so the caveat is accurate there by construction, not just because its spec locator now resolves. But it is *more* warranted there: on one small screen the **live** final-bid figure (reactive, `state.ui.output`) and the **frozen** agent options sit within a single short scroll — a tighter juxtaposition of the same asymmetry than desktop's Tab 7/8 split gives. So Phase E inherits this as a mobile case too, not only the original desktop one. The caveat text is shared across `AgentPage.jsx` and `BidSummaryPage.jsx`; removing it when the underlying staleness is fixed clears both places at once.
