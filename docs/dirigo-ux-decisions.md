...
- **Suite baseline: 33 spec files / 49 Playwright tests, 98 Vitest tests.** This expanded suite, including the three gap-closing specs plus the console-error and inverted regression specs, is the parity contract for A2.
...
**Finalize selection is not persisted — scheduled for A2.5.**
`submitBid()` independently recomputes the bid price from the plain calculator, discarding whichever option or custom override the user selected in the finalize modal. Verified against both a standard option and a custom override — the same incorrect amount is saved in each case.

Severity is higher than it first appears, because it writes wrong data rather than merely displaying it:

- Stored bid amounts feed `computeMarginOutcomeCurve()` and the margin-band win rates. Every historical margin figure is therefore computed against a price that was never submitted. Phase F's Insights view would render a well-tested chart built on numbers that did not happen — worse than having no chart.
- It blocks the *value* of Phase E, not just its features. Override reason capture (5.5) records why someone deviated from a recommendation that is currently discarded. Recommended-versus-chosen (5.6) has nothing to compare. Expected value (5.1) is decision support for a choice that evaporates on submit.

**Decision: fixed as a standalone change immediately after A2 merges, before Phase B.** Cheap once state lives in React, and every subsequent phase assumes stored bid amounts are meaningful.
...
|---|---|---|
| **A1 — Tooling & design system** | Vite, CSS layering, 6.1, 6.2, 6.3, 6.7, docs, `CLAUDE.md`, gap-closing specs | App stays vanilla. No module conversion (see §9.8). |
| **A2 — React migration** | Spike (Rates + History) → full migration → parity gate. Absorbs 3.2, 6.4. Module export conversion happens here. | No new features. No behaviour change. |
| **A2.5 — Finalize persistence fix** | Single defect from §9.9 | Standalone, before B |
