...
**2.2 Step completion state + URL routing — APPROVED. Implemented (Phase C, Step 1, 2026-09-01). The completion half was replaced 2026-09-07 — see 2.7.**
...
**2.6 "Open bids" list in the left nav — APPROVED. Implemented (2026-09-06, standalone brief, not a phase).**
The single "Current bid" nav item (2.5) is a route back to *one* draft; the only way to see the rest of the open work in the nav was not there — you had to open the header's "Open Bid ▾" combobox. This replaces that one item with a list: one `.nav-item` row per open draft, always visible in the nav.

No backend or data-model change — same store the header combobox and the Bids list already use. "Open" = every draft in `getAllDrafts()` (`dirigo_drafts`); a finalized bid is removed from that map by `clearFinalizedDraft()` (`js/forms.js`), so the map *is* the in-progress set — no separate filter invented. Rows sort by `lastModifiedAt || createdAt` desc, the same order as `OpenBidMenu`; the sort now lives in one shared `sortedOpenDrafts()` in `AppShell.jsx` that both surfaces call. Each row reuses the existing `.nav-item` markup/icon and collapses to icon-only via the existing `.nav-label` rule. The active draft is one of the rows (never shown twice), marked `.active` while the workflow is showing, with its label read from live reducer state so an in-progress rename appears before autosave flushes; other rows show their stored name. Click: the active row dispatches `GOTO_SECTION 'workflow'` (resume the step you left off, exactly as the old single item did); any other row calls `window.switchToDraft(id)` — the same call the header combobox makes, which navigates into the workflow itself.
...
**2.7 Manual tab-confirmation indicators — APPROVED. Implemented (2026-09-07, standalone brief, not a phase). Replaces 2.2's completion heuristic.**
2.2 derived each tab's Gray/Yellow/Green from field presence. A calculation always runs, so `ui.output` was non-null immediately and Cost Summary showed a false green on a blank bid; more broadly, "fields are non-blank" is not "the estimator has reviewed this." The new model is explicit:

- Each input tab carries a **"Finished with this tab"** button (`src/components/TabConfirmButton.jsx`, first child of every page's `.page-actions`). Cost Summary has no inputs of its own, so its button reads **"Reviewed"** and is eligible once a calc has produced a real number (`ui.output.summary.directCostTotal > 0` — not merely "an output object exists", since the reactive calc emits a $0 one the instant a blank bid loads and that must still read gray). Bid Strategy has **no button** — its indicator is derived from request state + `agentStaleness()` (nothing sent → gray, in flight → amber, cached+fresh → green, cached+drifted → amber).
- Clicking stores `JSON.stringify(normalize(<that tab's owned bid slice>))` as a snapshot in `bid.tabConfirmations[tab]` (`SET_TAB_CONFIRMATION`). Green = confirmed AND the live slice still matches the snapshot; editing a confirmed tab reverts it to amber for free (nothing watches — `tabStatus()` just stops matching on the next render), and editing the value back returns it to green.
- `normalize()` (sort keys, `String()`-coerce leaves) makes the snapshot stable across the `"12"` → `12` drift a save/reload round-trip introduces via `collectFormData()`'s `num()`.
- **Row tabs read `ui.output.state`, not `bid`** — Assemblies/Walls/Ceilings row fields are uncontrolled and never hit `bid` on a keystroke; only the reactive calc's `ui.output.state` moves. So those tabs are `empty` until the first calc, and a row-cell edit reverts them ~500ms later (the recalc debounce). On a browser reload they flash gray→green while `ui.output` recomputes, ~sub-second, while the five field tabs resolve instantly — accepted; the mitigation if it annoys is to also persist a row fingerprint on the draft.
- Field-tab eligibility requires **every** key in the slice non-blank, `0`/`null`/`false` included (`fieldFilled()` — "not blank", not "truthy"); the required list is the live slice's own keys, so there's no second hand-maintained list. `conditions` excludes `confidence`/`notes` (they belong to Market Read) and `durationWeeks` (a computed copy of the Project field that `collectFormData()` drops into `conditions` too); the `market` slice excludes `intelligence.openDraftCount` (a live count of other drafts) for the same reason — both would otherwise re-appear via autosave and false-revert a confirmed tab on the next reload. `rates` eligibility excludes the optional `rateEscalation` but the snapshot includes it. Free-text fields (`address`, `drawingsRef`, `exclusions`, `knownCompetitors`, every intelligence dropdown) are mandatory too — deliberate, to force explicit acknowledgement; if it proves to be too much friction the fix is a one-line per-tab exclusion set.
- **Send to Agent** (Market Read → Bid Strategy) is disabled until all 8 input tabs read green; its `title` names the outstanding tabs. Editing any confirmed tab re-locks it — the core "don't fire the expensive agent on a half-baked bid" behavior.
...
**4.3 Top cost drivers panel — APPROVED. Implemented (Phase E, Step 6, 2026-09-02).**
...
New `TopCostDrivers` component in `OutputPage.jsx`, rendered inside `Phase3` after Category subtotals (`.top-cost-drivers`). Pure display over `state.ui.output` — flattens `wallCosts` + `ceilCosts` (skips `.error` rows and zero-total rows), tags each Wall/Ceiling, sorts by `.total` desc, takes the top 5; each row shows location, kind · typeId, `$total`, and `% of summary.directCostTotal`. Then "Labor as % of direct cost" = `summary.laborTotal / summary.directCostTotal`. The five rows deliberately don't sum to 100% — logistics and markup aren't line items. No `calculator.js` change, no new calculation path. Returns null before the first calculation. Standalone — no dependency on the agent work. Pinned by `tests/e2e/top-cost-drivers.spec.js` (≤5 rows, dollar-descending, each row's % and the labor % cross-checked against the totals bar the page renders; absent with no calculation).

---

## 5. Decisions — Agent

**5.1 Expected value column — APPROVED. Implemented (Phase E, Step 3, 2026-09-02).**
...
New `src/state/expectedValue.js` (pure): `winProbBand(label)` maps each of the 5 likelihood labels to a **wide** P(win) band (Q2 = A, decided with Eric — Very High 70–90%, High 55–75%, Medium 40–60%, Low–Medium 25–45%, Low 10–30%; the band width is itself the honesty signal, given zero calibrated outcomes exist yet); `marginDollars(opt)` = `bidAmount × margin%`; `expectedValueRange(opt, label)` = band × margin$. `AgentPage.jsx`'s `OptionCard` renders `EV $lo–$hi` (never a point value) with an always-visible caveat line under the cards: "Expected value = win-likelihood band × margin $. Win-likelihood is a hand-tuned score, not a calibrated probability — treat EV as directional, not precise." For the seed the three EVs land close together ($42–55k / $44–61k / $41–61k) — exactly the "not comparable by eye" case 5.1 exists for. Shared with 5.3's slider (Step 4) so its endpoints reproduce each card's EV. Pinned by `tests/e2e/agent-expected-value.spec.js` (range format on every card; the caveat; competitive EV anchored to `$42,493–$54,634` = the band constants × demo values).

**5.3 What-if price slider — APPROVED. Implemented (Phase E, Step 4, 2026-09-02).**
...
New `src/components/WhatIfSlider.jsx`, rendered in the "Bid options" section **below** the cards — a sibling of the card row, not a child of any `[data-bid-opt]` card, so a drag/click can't fire `SELECT_AGENT_OPTION` (`stopPropagation()` on the range and anchor buttons too, defensively — per Eric's Step 4 note). `interpolateAtBid(options, x)` in `src/state/expectedValue.js`: at `x` exactly equal to an anchor's `bidAmount` it returns that option's own `{ marginPct, winBand, winLabel, ev }` **verbatim** (no lerp) so the endpoint match is dollar-exact; between anchors it linearly interpolates `margin%` and the P(win) band endpoints between the two bracketing anchors. Readout: `$price`, `N.N% margin`, `Win likelihood <label>` at an anchor / `~lo–hi%` between, `EV $lo–$hi`. Range `min`/`max` = the min/max option amount, `step` 100, starts at the recommended amount; three clickable anchor markers. Pure local state — sets nothing on the bid, persists nothing, doesn't feed finalize. Mobile: 44px range hit area + 26px webkit thumb (`css/responsive.css`). Pinned by `tests/e2e/agent-what-if-slider.spec.js` (renders below the cards, starts on recommended; at each of the three anchors the readout — incl. EV — equals that option card's text exactly; between anchors margin lands strictly between the bracketing options and win shows a `~lo–hi%` band; moving the slider leaves the finalize selection on `recommended`).

**5.2 Win-likelihood attribution — APPROVED. Implemented (Phase E, Step 3, 2026-09-02).**
...
`js/agent.js`'s `deriveWinLikelihood()` was refactored **in place** (Q4 = approved — no duplicate scoring table in `src/`, which would recreate the `ASM_DEFAULTS` coupling hazard) into `deriveWinLikelihoodBreakdown(intelligence, optionType)` — a data table (`_WIN_LIKELIHOOD_FACTORS`) + `_winLikelihoodLabel()` — returning `{ label, score, base, contributions: [{ factor, value, delta, direction }] }`. `deriveWinLikelihood()` is now a thin `.label` wrapper; `_demoResponse()` and any live path are unchanged (the factor values are mutually exclusive per signal, so table lookup == the original if-chain). Exposed as `window.__winLikelihoodBreakdown` (guarded per checklist item 9). In `AgentPage.jsx` the pill is a `<button>` (`stopPropagation` so it doesn't also select the card) that toggles a small table: each of the four factors, its current value, and ▲ helps / ▼ hurts / – neutral with the delta. Pinned by `tests/e2e/agent-expected-value.spec.js` (per-card independent expand; base/score/label header; seed factor values + directions; pill click doesn't change the finalize selection).

**5.5 Override reason capture — APPROVED. Implemented (Phase E, Step 5, 2026-09-02).**
...
`FinalizeModal.jsx` shows a `#finalize-reason-block` (label "Why not the recommended bid? (optional)", the five chips as multi-select toggle buttons, a free-text `<textarea>`) **whenever `selected !== 'recommended'`** — a non-recommended standard option *or* a custom override, per §9.9's "when the chosen option differs from the recommended one". Both parts are optional; neither gates Confirm. Reducer state `finalizeModal.reasonChips` / `reasonText` (`TOGGLE_FINALIZE_REASON_CHIP` / `SET_FINALIZE_REASON_TEXT`), reset by `OPEN_FINALIZE_MODAL`. `handleConfirm()` passes `overrideReason: { chips, text }` (only when non-recommended) through `submitBid()`; `buildBidRecord()` stores `override_reason_chips` (string[], click order preserved) and `override_reason_text` (string) — **`[]` / `''` when recommended was chosen or no reason given, never null**, so consumers treat them as a list and a string unconditionally. Bid-record-only; golden-export byte-identical. Hand-verified against a real captured record (`tests/e2e/agent-override-reason.spec.js`): Competitive + two chips + text → `selected_option:'competitive'`, `final_bid:271000`, `recommended_bid:284500`, `override_reason_chips:['need the work','competitor intel']`, `override_reason_text:'<note>'`; plus block-visibility per selection, optionality (Confirm not gated), recommended → empty fields, and reset-on-reopen.

**5.6 Confirmation panel on the correct tab — APPROVED. Implemented (Phase E, Step 1, 2026-09-02).**
Render the submitted panel on Tab 8 where finalize happens, and add what it currently lacks: recommended versus chosen.

`OutputPage.jsx`'s inline success/failure panel is now a shared `src/components/SubmitResultPanel.jsx`, rendered from **both** `OutputPage.jsx` and `AgentPage.jsx` off the same `state.ui.submitResult` (Q3 = A, decided with Eric — not a shell-owned overlay, which would add a fourth uncoordinated fixed-position element to the ad-hoc z-index pattern §9.9 already flags). Both real finalize origins are covered — Agent's "Finalize bid →" and Output's post-failure "Try again" — and the visible copy is whichever page is `.active` when the submit resolves. No `id` on the wrapper (it's in the DOM twice, both pages always mounted); `.submit-result-panel` class is the test hook. The panel still self-clears on the next `RENDER_OUTPUT`, unchanged. Recommended-vs-chosen shows on the success view: `Recommended $X · you bid $Y (±$Z, ±N%)`, or "Matched the recommended bid." `buildBidRecord()` gains `recommended_bid` / `recommended_margin_pct` (the recommended option's own figures at confirm time, threaded through `FinalizeModal.handleConfirm()` → `submitBid()`), so the comparison is persisted on the record for later analysis, not only rendered live — bid-record-only, golden-export byte-identical (that flow never calls `submitBid()`). Pinned by `tests/e2e/finalize-confirmation-tab.spec.js` (failure panel visible on `#page-agent`, present-but-hidden on `#page-output`; success panel + recommended-vs-chosen on `#page-agent`; persisted `recommended_bid`/`recommended_margin_pct`).
...
**6.9 Data-input tab UI-fixes batch, 2026-09-03 (branch `ui-fixes-batch-2026-09`).** Four items from a UI/spacing audit of the Project → Market Read tabs. No business-logic file touched, golden-export byte-identical, full Playwright suite green unmodified + one new spec (`date-field.spec.js`). Each item had an explicit trade-off decided with Eric at plan approval (OQ1–OQ5 below); recording the rationale here so it isn't lost.

- **Sub-group spacing hierarchy (Rates + Market Read).** `.sub-lbl` (which had `margin-bottom` but no `margin-top`) gained `margin-top:4px`, and specific second-and-later sub-labels within a group ("Drywall board by type" on Rates, "Competitive signals" on Market Read) get an inline `marginTop:28` so sibling sub-groups don't run together the way the audit flagged. A new "Finishing materials" `.sub-lbl` was added above the previously-unlabelled Tape/Insulation/Fasteners `.rgrid` on Rates — a real labelling gap, not only spacing.
  - **OQ1 → leave `.rgrid` `margin-bottom` alone** (the audit brief proposed 12 → 16px). It's shared with `OutputPage.jsx` and most Rates `.rgrid`s already set an inline `marginBottom` that overrides it, so the change would have moved ~4px on one out-of-scope page for no real gain. The sub-label margins carry the fix alone.
  - **OQ2 → accept the mobile cost.** The 4/28px additions apply at every width (the mobile collapse rules only touch `grid-template-columns`), adding ~100–150px of vertical scroll on an already-long single-column Rates page on a phone. A `@media` scale-down was rejected — Rates is desk-oriented data entry, not worth a second mobile-only spacing model to keep in sync.

- **Type ID / Stud size column width (Walls / Ceilings / Assemblies).** `TypeIdSelect`'s enriched option label (`W#: stud / board / L#`, added in Phase B §3.1 so estimators can tell assemblies apart without leaving the tab) was clipping in a 64px `<col>` sized for a bare 2-char ID — a pure rendered-width bug, which is why the option-text spec never caught it. The three colgroups' inline `<col style={{width}}>` for that column became `<col className="col-typeid" />` / `col-studsize`; width moved to `css/pages.css`.
  - **OQ3 → responsive width via a class, not an inline pixel bump, and not per-phone text abbreviation.** Desktop `182px` / `98px` (`css/pages.css`); a `@media (max-width:768px)` override in `css/responsive.css` holds them near the original `66px` / `84px` on a phone, where a tapped `<select>` opens the OS picker showing full option text regardless of column width — so the wider desktop column would only add horizontal scroll there for no readability gain. Abbreviating the option text itself on mobile (`TypeIdSelect` unchanged) was rejected as speculative.
  - **182, not the brief's "~170".** Verified by screenshot: 170 still clipped the longest label (`W#: 3-5/8" / Standard / L3` — "Standard" is the widest board type). 182 clears it without widening the row enough to matter.
  - **Location column left at 150** — not shrunk to fund Type ID. Location (floor/zone) is cross-referenced right alongside Type ID when scanning, and `.tbl-wrap` already scrolls horizontally, so the row is allowed to grow instead.

- **Site Conditions field consistency (`ConditionsPage.jsx`).** Its local `Field` helper gained an optional `suffix` prop → a muted `<span className="unit-sfx">` after the input in a `.field-with-sfx` flex row (new classes, `css/pages.css`). Applied consistently: `%` on Waste factor override, `LF` on Curved walls LF, `phases` on Phase count. Each of the three also gained a `.rhint`. Estimated delivery trips moved from a bare `.field` (which read as detached from the site-flags grid above it) into a `.flag-card` with a hint.
  - **OQ4 → one lightweight text suffix, not the Rates `.iw`/`.isfx` bordered box.** The Rates pattern puts the suffix in a bordered input container; dropping that into a `.flag-card` next to the plain `<select>`s already there would look like a mismatched import from another page. Hint-only (no visible unit) was also rejected for the waste field specifically — "Waste factor override" doesn't self-evidently read as a percentage.
  - **Waste-factor hint copy was source-checked, not paraphrased from the audit brief, which had the relationship backwards.** The field is the **job-wide** waste % on board material (`effectiveWaste = asm.wastePctOverride ?? conditions.wastePct`, `js/calculator.js`); blank falls back to **10** (`js/state.js:131`); per-assembly overrides on the Assemblies tab take precedence over it. It is *not* an override of "the calculated waste % from assembly weighting" — that weighted figure (`weightedWastePct`, shown on Cost Summary) is a derived output. Copy: *"Job-wide waste % on board material. Blank = 10%. Individual assemblies can override this on the Assemblies tab."*

- **Date fields → guaranteed US MM/DD/YYYY (`src/components/DateField.jsx`, new).** Native `<input type="date">` renders in the *browser's* locale, so on a non-US machine the Project tab's Bid due date / Est. start date showed DD/MM/YYYY. `DateField` is a controlled `<input type="text" inputMode="numeric">` displaying MM/DD/YYYY, with `isoToUs()` / `usToIso()` helpers (no date library) and blur validation — an unparseable non-empty entry keeps its text, gets `aria-invalid` + a `.datefield-input.invalid` red border, and does **not** propagate (the stored value is untouched).
  - **OQ5 → one `DateField` at every viewport**, including mobile. Rendering the native `type="date"` on small screens (to keep the OS calendar wheel) would reintroduce the exact locale bug this closes, just scoped to mobile — and defeats "guaranteed, not best-effort". Phone users type MM/DD/YYYY on the `inputMode="numeric"` pad; not worth two code paths and split test coverage.
  - **Storage stays ISO — no data-model or export change.** A hidden `<input id={id}>` with `value={get(['project', <field>])}` (a pure reflection of reducer state, not a separate store) carries the `YYYY-MM-DD` value that `js/state.js`'s `sel('proj-bid')` / `collectFormData()` and the export payload read. `onChange` dispatches `SET_FIELD` exactly like every other Project field, so reducer state stays in sync; `populateForm()` / `resetFormFields()` keep the existing "direct `set()` + dispatch, both always" hydration shape. `focusedRef` (a ref, not state) gates the prop→display sync effect so a blur can't wipe an in-progress invalid entry. Verified against the running app: typed value reaches reducer state, `collectFormData()`, and persisted autosave identically, with no console warnings. `BidsToolbar.jsx`'s history-filter date inputs were left native (out of audit scope).

**Follow-up (same day, branch `rates-taping-adders-width`).** Rates "Taping + finishing" section column-aligned to the 3-up `.rgrid.g3` (Labor rates) above it, per direct request: `.taping-adders-row` became `1fr 1fr 1fr` with the finish-level table (`.taping-col`) spanning 2 columns and the height adders taking the 3rd — same 10px gutter as `.rgrid`, so the edges line up exactly (was `minmax(0,1fr) 340px` / 24px gap). Added a `@media (max-width:768px)` stack for the row (the desktop 2-span would crush the finish-level table to ~1/3 of a phone; it wasn't in the existing grid-collapse list). CSS only, no spec touches this layout, full suite green.

**Deliberately out of scope for this batch** (both already tracked in §9.9): the Rates totals-bar summing unit-mismatched values under "Direct cost total", and the Plastering / External-wall rate fields being non-functional orphans with no "not yet costed" signal. Both need a product-scope discussion before anything is touched.

Cross-refs: §3.1 (Type ID dropdown), §6.1 (type scale), §7 (mobile framing / the accepted OQ2–OQ3 mobile trade-offs), §9.9 (the two deferred items).

**6.10 Responsive tray columns, 2026-09-05 (branch `responsive-tray-columns`).** The Material Rates / Labor Rates / Site Conditions trays doubled two sub-groups into one `.tray-col` (a second `.sub-lbl` with an inline `marginTop:28`, added in §6.9). Each sub-group now gets its own `.tray-col` — Material Rates: Stud+track / Board types / Finishing; Labor Rates: Base rates / Height adders / Taping; Site Conditions: Site flags / Height zones / Delivery — and `.tray-cols` changed from `display:flex` to CSS Grid `repeat(auto-fit, minmax(min(var(--col-min,200px),100%), max-content))`, so columns wrap 3 → 2 → 1 as the tray narrows with no JS, no breakpoints, no `ResizeObserver`. Pure CSS; no component logic. Short/uneven columns (Delivery = 1 row, Height adders = 2) are accepted, not rebalanced.
- **`max-content`, not `1fr`, on the upper bound.** `1fr` would stretch every tray's columns to fill leftover width, changing the "2-column" trays (Logistics/Markup, Market signals/Competitive signals) that were deliberately built to sit at natural width and centre as a compact block. `max-content` preserves that — verified: those trays' DOM geometry and screenshots are byte-identical before/after. Each has a single `.tray-col`, so it renders as one centred grid track exactly as it did as one centred flex child.
- **`--col-min` is `var(--rr-width)`**, the per-page uniform row width `useUniformRowWidths.js` already measures (≈314px Rates, ≈312px Site Conditions), inherited into `.tray-cols`. Self-adjusting per page — no invented number, no per-tray value to keep in sync. The literal `200px` is only the pre-`useLayoutEffect` first-paint fallback.
- **Gap dropped 64px → 40px.** 64 was inherited from the 2-column design and never validated for 3. Measured: with `--col-min`≈314, 3-up needs `3×314 + 2×gap` of `.tray-cols` width; at 40px that's 1022px, which holds 3-up at a maximized laptop (1440 → ~1118px available, margin ~96px) and collapses to 2-up by ~1340 and 1-up by ~980. One universal value — the gap only ever applies between the multiple columns of these three trays (every other tray has a single `.tray-col`).
- **Mobile (`css/responsive.css` ≤768px) override kept, rewritten for grid:** `.tray-cols { grid-template-columns: 1fr }`. `auto-fit` collapses to one *track* on its own at narrow widths, but `minmax(col-min, max-content)` would size that track to its content, not the full tray — `1fr` forces the full-width single column the mobile layout needs (the end the old flex `align-items:stretch` achieved).
- **Supersedes §6.9 / OQ2 in part.** OQ2 accepted the `marginTop:28` sub-group separators' vertical cost on mobile; those inline styles are removed here (the sub-groups are separate columns now), so on a phone the three now stack with the uniform grid `gap:4px` between them rather than 28px within a column. Deliberately left uniform — they are independent columns now, not "two groups that used to be stacked"; no per-tray exception (see also the removed special-casing rationale in the branch's plan).
- New spec `tray-columns-responsive.spec.js` (4 tests): live-resize 3→2→1 with no body overflow, Site Conditions 3-up at laptop width, the baseline trays staying a single track, and every tray one full-width column below 768px. Vitest unchanged (no JS logic touched); golden-export not in play (no `submitBid()` path).
...
_Phase D Step 3 (2026-09-02) — shipped:_ **Log outcome** needed no new logic — `BidUpdateRow.jsx` already captures outcome / competitor / winning bid (+ actual costs, notes) and persists via `window.updateBid`; Step 2's `.grid.g6` stacking + one CSS rule (pin the expanded `colSpan` cell to the viewport's left edge, clamp its form width) make it usable on a phone while the wider Bids table is scrolled. **Bid Summary** is a new read-only page (`src/pages/BidSummaryPage.jsx`) — a live projection of `state.bid` + `state.ui.output` + `state.ui.agent` for the **current** bid, showing everything §7 lists (quantity totals = wall LF + ceiling SF, summed from `state.ui.output`'s per-area rows). Per **Q2** it is the current bid, not a per-submitted-bid view: `buildBidRecord()` doesn't store the agent options / risk flags / quantity totals, so a per-submitted summary is a recorded Phase E-dependent candidate (§9.9). Reached only from a **mobile-only drawer item** (`.nav-item-mobile`, CSS-hidden ≥768px) and the `#/summary` route — same `activeSection` + router pattern as Phase C's `biddecision`; "reached in-app only" (below) is satisfied by there being no desktop affordance and no share link. Carries the same stale-inputs caveat `AgentPage` does. Pinned by `mobile-layout.spec.js` (drawer reach, current-bid projection, return-to-workflow, desktop item hidden; log-outcome form stacks within the viewport with 44px buttons).
...
**Implemented — Phase F, Step 1 (2026-09-05).** `src/pages/InsightsPage.jsx`, a standalone read-only screen (new `activeSection` `'insights'` + `#/insights` route, its own left-nav item — Q1 at plan review) rendering three sections: **win rate by margin band** (`computeMarginOutcomeCurve`, gated at 15 decided bids — below that, an `.empty-state`), **win rate by quarter** (`computeSeasonality`, no gate, with a low-volume note under ~8 decided bids), **competitor loss patterns** (`computeCompetitorPatterns`, a `.tbl-wrap` table; `avgUndercutPct` shows `-` until 2 priced losses to that competitor). All three consume the existing `window.*` exports verbatim — no `history-analytics.js` change, no new math. **Charting: CSS-only bars, no dependency added** — ≤ 8 bands, and §9.1's "no dependency rot" value; new `.insight-*` classes in `css/components.css`, sections in `.tray` / `.tray-cols` so they inherit §6.10's 3→2→1 reflow. On the current 5 seed bids (4 decided) the margin curve renders its "not enough data" state by design — that is the realistic primary state and is tested as such. Data via `window.getAllBids()` (same tri-state as `BidsPage.jsx`). `tests/e2e/insights.spec.js` covers both the gated seed state and a route-mocked 18-decided populated state. Golden-export unaffected (read-only).
...
**Agent-option display can go stale before the user confirms — RESOLVED (Phase E, Step 2, 2026-09-02). Q1 = B: warn + acknowledge + record.**

The fix, as built:
- **Detect.** `js/ui.js` now fingerprints the calc state each agent run is launched against (`_agentCalcFingerprint = { bidPrice, directCost }`, set in `_launchBidAgent()` and `runAgentIfNeeded()`, cleared by `_resetAgentCache()`), carried into `state.ui.agent.generatedAt` via `window.__renderAgentTab`. `src/state/agentStaleness.js` (`agentStaleness(state)`) compares it against the live `state.ui.output`, returning `{ stale, bidPriceDelta, directCostDelta }`.
- **Threshold — OR, not the plan's first-pass AND.** `stale = |Δbid| > $500 OR |Δbid|/bid > 0.5%`. The AND form was sanity-checked against the demo's ~$284k bid and rejected: 0.5% there is ~$1,420, so the relative arm dominated and a real ~$1k line-item edit wouldn't have tripped the warning. OR with a $500 absolute floor reads as "the bid moved more than $500, or more than 0.5%" — how an estimator thinks about a meaningful change — and the floor still filters rounding noise. Recorded in `agentStaleness.js`'s header.
- **Warn.** The Phase B interim one-liner is gone from **both** `AgentPage.jsx` and `BidSummaryPage.jsx`. In its place, a shared `src/components/AgentStalenessWarning.jsx` renders **only when `stale`** — the actual gap ("Current bid price $X vs $Y when the agent ran (Δ ±$Z)"), plus a "Re-run agent" action on the pages (not in the modal) with an explicit demo-mode note that re-running refreshes the check, not the fixed option amounts.
- **Acknowledge + gate.** `FinalizeModal.jsx` shows the same warning above the footer plus a `#finalize-stale-ack` checkbox; `confirmDisabled` composes `(staleBlocking && !staleAck)` with the existing override-amount gate — neither OR-passes for the other (pinned by `agent-staleness.spec.js`'s combined-gate test: valid amount + un-acknowledged still blocks; acknowledged + missing amount still blocks). New reducer state `finalizeModal.staleAck` + `SET_FINALIZE_STALE_ACK`, reset on `OPEN_FINALIZE_MODAL`.
- **Record.** `buildBidRecord()` gains `inputs_stale_at_submit` (bool), `stale_bid_price_delta`, `stale_direct_cost_delta` — captured every submit where an agent result exists, not only when non-zero, so "acknowledged, and the gap was $X" is a usable calibration signal. Bid-record-only; golden-export byte-identical.
- **Spec.** `agent-staleness-caveat.spec.js` → `agent-staleness.spec.js`, flipped from "passive caveat present" to: fresh result shows no warning (and the old always-on text is gone); an input edit surfaces the active warning with the real gap; "Re-run agent" clears it; the acknowledge checkbox composes with the override gate; a stale finalize records the ack + delta; a non-stale finalize has no checkbox and records `false`.

**Original writeup (kept as history):** The agent's competitive/recommended/ambitious `bidAmount`s (what the finalize modal displays and what the user picks from) come from `_lastCalcSum`/`_lastCalcMarkup` — module-level snapshots set whenever the agent last ran (`js/ui.js`, `_launchBidAgent()`/`runAgentIfNeeded()`). If the user edits Rates, Assemblies, Walls, Ceilings, or Markup inputs *after* the agent last ran but *before* opening the modal and clicking Confirm, the modal displays option amounts computed against stale inputs — a number the user never actually re-derived. The A2.5 fix makes this slightly more concrete than before: it now persists the *displayed* amount verbatim at confirm-time, so a stale display is faithfully saved as `final_bid`, rather than being silently overwritten by a fresh (also-wrong-for-different-reasons) recompute the way the pre-fix code accidentally was. Concrete failure scenario: open Tab 8 (agent runs, options computed from current inputs), go back and change a wall quantity or a markup %, return to Tab 8 without re-running the agent, click Ambitious, confirm — the saved bid reflects the *old* inputs, not the ones now on the form.
...
**Interim caveat shipped (2026-08-28, Phase B, standalone commit).** Eric's call, made directly rather than left to Phase E: "These options may reflect an earlier version of your inputs." now renders above the Bid options cards (`AgentPage.jsx`). Reasoning recorded here so it isn't mistaken for scope creep later — this wasn't Phase B stumbling onto an old bug and being asked to leave it alone; Phase B's own reactive-calc change (4.2) is what turned a dormant inconsistency into an actively misleading one within a single session (see the checkpoint verification above: a $53k–80k gap between the rail and the frozen cards). Phase B made it worse, so Phase B shipping a cheap mitigation is fair scope, even though the real fix (re-validate/relaunch the agent against fresh inputs) stays Phase E's job exactly as decided. Scope deliberately minimal — one line, no visual redesign, no gap quantification, no interaction — pinned by `tests/e2e/agent-staleness-caveat.spec.js`. **Phase E: a passive mitigation already exists when you arrive to build the real fix — the caveat can be removed once the underlying staleness is actually resolved, not left stacked on top of it.** — _Done: Phase E Step 2 (2026-09-02) removed the interim caveat from both `AgentPage.jsx` and `BidSummaryPage.jsx` and replaced it with the active detect-warn-acknowledge-record fix above; spec renamed `agent-staleness.spec.js`._

**Visibility update (2026-09-02, Phase D Step 3) — same "the phase's own change made this more visible" pattern as the 2026-08-28 note above, now on a second surface.** Phase D's mobile Bid Summary (`BidSummaryPage.jsx`) reads the identical `state.ui.agent.cachedResult` object `AgentPage` does, through the same shared store, and carries the same one-line caveat above the same stacked options — so the caveat is accurate there by construction, not just because its spec locator now resolves. But it is *more* warranted there: on one small screen the **live** final-bid figure (reactive, `state.ui.output`) and the **frozen** agent options sit within a single short scroll — a tighter juxtaposition of the same asymmetry than desktop's Tab 7/8 split gives. So Phase E inherits this as a mobile case too, not only the original desktop one. The caveat text is shared across `AgentPage.jsx` and `BidSummaryPage.jsx`; removing it when the underlying staleness is fixed clears both places at once. — _Done: Phase E Step 2 replaced it in both places with the shared `AgentStalenessWarning` component (active, shown only when `agentStaleness()` detects drift), incl. on `BidSummaryPage` — pinned by `agent-staleness.spec.js`'s "BidSummaryPage carries the same active warning" test._
...
**Finalize failure panel renders to the wrong tab — RESOLVED (Phase E, Step 1, 2026-09-02).**
The submitted and failure panels rendered into `#output-bid` on Tab 7 while finalize happens on Tab 8, so the durable confirmation was invisible and only the toast was seen. Preserved as-is through Phases A–D. Fixed in 5.6: the panel is now a shared `SubmitResultPanel` component rendered from `AgentPage.jsx` as well as `OutputPage.jsx`, both off `state.ui.submitResult`, so it appears on whichever tab is active when the submit resolves. See §5.6 above. Pinned by `tests/e2e/finalize-confirmation-tab.spec.js`.
...
| **D — Mobile** 🔨 | Tier 1, sticky columns, `100dvh`, Tier 2 | **All 3 build steps done 2026-09-02 on `phase-d-mobile`, pending deploy-preview + merge.** (1) structural: drawer nav, scrollable step bar, 16px inputs, demo hidden, 44px targets, `100dvh`, mobile header trim; (2) layout: grid stacking, `.page` padding, totals 2×2, sticky first column on the takeoff/Bids/Cost-Summary tables (scroll-shadow as a gradient, not `box-shadow`); (3) Tier 2: `BidSummaryPage.jsx` (current-bid rollup, mobile-only drawer item + `#/summary`), Log outcome reuses `BidUpdateRow` unchanged. Pinned by `mobile-layout.spec.js` (20 tests). Q2: the summary is the **current active bid** (a per-submitted-bid summary needs `buildBidRecord()` snapshots — deferred, §9.9). No `calculator.js`/`history-analytics.js` change; golden-export byte-identical. |
| **E — Agent** ✅ | 5.1, 5.3, 5.2, 5.5, 5.6, 4.3, agent-option display-staleness fix (§9.9) | **Complete 2026-09-02** on `phase-e-agent`, 6 build-order steps, production + test/docs commit pair each. Decisions with Eric: Q1→B (staleness = warn + acknowledge + record deltas), Q2→A (wide EV bands), Q3→A (shared panel not overlay), Q4→refactor `deriveWinLikelihood()` in place. **Step 1 (5.6)**: shared `SubmitResultPanel` on Agent+Output, recommended-vs-chosen, `recommended_bid`/`recommended_margin_pct` on the record; wrong-tab §9.9 defect closed. **Step 2 (staleness §9.9)**: `agentStaleness.js` + `AgentStalenessWarning` (OR threshold $500/0.5%, changed from the plan's AND after a sanity-check), interim caveat removed from `AgentPage` + `BidSummaryPage`, acknowledge checkbox composes with the override gate, `inputs_stale_at_submit`/`stale_bid_price_delta`/`stale_direct_cost_delta` on the record; staleness defect closed; spec → `agent-staleness.spec.js`. **Step 3 (5.1+5.2)**: `expectedValue.js` (wide bands), `deriveWinLikelihood()` refactored in place to a breakdown, EV range + always-visible caveat + click-to-expand attribution. **Step 4 (5.3)**: `WhatIfSlider.jsx` + `interpolateAtBid()` (endpoint-exact), below the cards, isolated from card selection. **Step 5 (5.5)**: reason chips + free text when `selected !== 'recommended'`, `override_reason_chips`/`override_reason_text` ([]/'' not null), hand-verified against a real record. **Step 6 (4.3)**: `TopCostDrivers` on Cost Summary, pure display. No `calculator.js`/`history-analytics.js` change; golden-export byte-identical throughout. Final: 98/98 Vitest, 151/151 Playwright (130 pre-phase unmodified + 21 new across 5 spec files). Pending: deploy-preview + merge (Netlify now team `dahmsio` / site `bid-iq`). |
