...
- Field-tab eligibility requires **every** key in the slice non-blank, `0`/`null`/`false` included (`fieldFilled()` — "not blank", not "truthy"); the required list is the live slice's own keys, so there's no second hand-maintained list. `conditions` excludes `confidence`/`notes` (they belong to Market Read); `rates` eligibility excludes the optional `rateEscalation` but the snapshot includes it. Free-text fields (`address`, `drawingsRef`, `exclusions`, `knownCompetitors`, every intelligence dropdown) are mandatory too — deliberate, to force explicit acknowledgement; if it proves to be too much friction the fix is a one-line per-tab exclusion set.
