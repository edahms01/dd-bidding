...
**As of the A2 spike, this app is a hybrid.** React (`src/`) owns the shell (header, left nav, tab bar) plus the Rates and History pages. Every other page (Project, Conditions, Assemblies, Walls, Ceilings, Output, Agent, Dashboard) is still the original vanilla JS/HTML described below, coexisting inside the same React tree via a wrapper component. **Read "A2: React migration — hybrid coexistence" further down before touching state, navigation, or any page's rendering** — the mechanism is load-bearing and non-obvious; changes that look safe in isolation (e.g. "just add a `<script type=module>`" or "just read `window.hasUnsavedChanges`") have already broken things once during the spike itself.
...
- `window.goto(id)` — replaces `js/tabs.js`'s `goto()`. Dispatches the tab change into the shared reducer and still fires the per-tab legacy side effects (`_renderPipelineHint()` on Conditions, `runCalculation()` on Output, `renderAgentTab()` on Agent) for pages that haven't converted.
...
- `window.__hydrateRates(rates, rateEscalation)` — the fix for a real hazard: `populateForm()`/a template load used to write straight into Rates' DOM inputs via `el.value = ...`. Those inputs are React-controlled now; a direct external `.value` write gets silently overwritten on the next re-render, since React never learns the DOM changed out from under it. `populateForm()`'s Rates section (`js/forms.js`) now calls this bridge instead, with the old `set()`-based code kept as a fallback for non-browser/pre-mount contexts.
...
**Boot ordering — a real bug found and fixed, not a theoretical concern.** `js/forms.js`'s old INIT section ran three things at classic-script-load time or on `DOMContentLoaded`: `addAsm()`/`addWall()`/`addCeil()` (default blank rows), the `.workflow-area` input/change listener (what actually drives autosave), and `_initDraftsAndResume()` (draft restore). All three assumed the page's real DOM already existed — true when every page was static HTML present the instant the parser reached it, **not true** now that `LegacyPage` clones its template in from a post-mount `useEffect`. `DOMContentLoaded` doesn't fix this either: module scripts run before `DOMContentLoaded` fires, but React's `useEffect` is scheduled for after paint — a race against it, not a guarantee of ordering. Reproduced directly: `addAsm()` threw on a null `#asm-body`, and separately (silently, no error) the autosave listener was simply never attached at all. Fixed with a `'dirigo:shell-ready'` event, dispatched by `AppShell` once every `LegacyPage` child's mount effect has run (children's effects fire before a parent's, in the same commit) — `forms.js`'s `_initApp()` listens for that instead of `DOMContentLoaded`. **Any future code that needs DOM from a not-yet-converted page to exist must wait for this event, not `DOMContentLoaded`.**

**Dead code from this spike, not yet deleted:** `js/ui.js`'s `renderRateTemplateSelect()`/`saveRateTemplateFromForm()`/`loadSelectedRateTemplate()`/`deleteSelectedRateTemplate()` and `js/forms.js`'s `applyRateTemplate()` — all reimplemented natively in `src/pages/RatesPage.jsx` (which calls the underlying `rate-templates.js` network functions directly rather than these DOM-writing wrappers). Verified nothing else calls them before leaving them alone. Same for `js/ui.js`'s `toggleUpdate()`/`renderHistory()` and friends, superseded by `src/pages/HistoryPage.jsx`. Slated for removal during the full-migration cleanup pass, not this spike — touching genuinely-unreachable code adds risk for zero behavior change.

**Two known pre-existing quirks the React port had to replicate exactly, not fix:**
- The Rates page has two real, orphaned inputs — Plastering and External wall (`#rate-plaster`/`#rate-extwall`). They're rendered, they have `oninput`/contribute to the totals bar, but `collectFormData()` never reads them and nothing persists them — confirmed by grep, not assumed. `RatesPage.jsx` renders them as plain uncontrolled inputs, not wired to any state, matching this exactly.
...
- **Current state**: Preflight, Step 0, and A1 (design tokens, still fully vanilla JS) merged to `main`, live in production. **A2 spike** (React introduced for the first time — shell + Rates + History, see the hybrid-coexistence section above) drafted and verified on branch `a2-spike-shell-rates-history`: full 49-test/98-test suite green unmodified, both required empirical gates passed (an unrelated re-render leaves an already-populated `LegacyPage`'s DOM byte-for-byte untouched; a single draft with Rates data entered through React and every other field through vanilla pages round-trips correctly through both export and reload), production `vite build` verified booting cleanly. Per the approved plan, stopping here to report before converting any further page — not yet merged.
